...
selectInput("age_at_latrem", label = "Age group at last removal: "
...
states@data <- states@data %>%
  ...
  filter(ffy == input$ffy, age_at_latrem == input$age_at_latrem) %>%
  left_join(ref_age_groups, by = c("age_at_latrem" = "cd_age"))
...
states@data <- states@data %>%
  inner_join(dat_legally_free, by = c("st_abbrev" = "st")) %>%
  filter(ffy == input$ffy2, age_at_latrem == input$age_at_lf) %>%
  left_join(ref_age_groups, by = c("age_at_latrem" = "cd_age"))
...
selectInput("age_at_latrem2", label = "Age group when entering care: "
            , choices = c("All" = 0, "0-4" = 1, "5-9" = 2, "10-14" = 3, "15-17" = 4)
            , selected = 0)
...
dat_legally_free %>%
  filter(ffy == input$ffy3, age_at_latrem == input$age_at_latrem2) 
